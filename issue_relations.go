...
func (c *Client) CreateIssueRelation(issueRelation IssueRelation) (*IssueRelation, error) {
	...
	req, err := http.NewRequest(http.MethodPost, c.endpoint+"/relations.json?"+c.apiKeyParameter(), strings.NewReader(string(s)))
	if err != nil {
		return nil, err
	}
...
func (c *Client) UpdateIssueRelation(issueRelation IssueRelation) error {
	...
	req, err := http.NewRequest(http.MethodPut, c.endpoint+"/relations/"+strconv.Itoa(issueRelation.Id)+".json?"+c.apiKeyParameter(), strings.NewReader(string(s)))
	if err != nil {
		return err
	}
	req.Header.Set("Content-Type", "application/json")
...
func (c *Client) DeleteIssueRelation(id int) error {
	req, err := http.NewRequest(http.MethodDelete, c.endpoint+"/relations/"+strconv.Itoa(id)+".json?"+c.apiKeyParameter(), strings.NewReader(""))
	if err != nil {
		return err
	}
	req.Header.Set("Content-Type", "application/json")
